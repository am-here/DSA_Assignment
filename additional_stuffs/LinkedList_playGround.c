...
void delete ()
{
    if (size == 0)
    {
        printf("Don't you know the Linked List is empty! Huhhh!\nWhat would I delete? Please continue and add an element!\n");
        return;
    }
    char ch;
    printf("\na. At the beginning\nb. At the end\nc. At any position\nEnter your choice: ");
...
void traverse_reverse(node *value)
{
    if (value == NULL)
        return;
    traverse_reverse(value->next);
    printf("%d ", value->data);
    return;
}
void search()
